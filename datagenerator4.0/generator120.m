...
Vc = [4.8, 24, 60, 80, 120];    %用户移动速度km/h
%type = 'CDL-A'
...
%合并后数据保存路径
filename2 = "../data/v120"+type+"_"+num2str(carrnum)+"_"+num2str(BSAtNum)+"_"+num2str(UEAtNum)+"_"+num2str(number)+".mat" %保存路径
...
for j = 1:epoch
    ...
    %每个epoch进行保存
    filename = "../data/v120"+type+"_"+num2str(carrnum)+"_"+num2str(BSAtNum)+"_"+num2str(UEAtNum)+"_"...
    +num2str(number)+"_ep"+num2str(j)+"of"+num2str(epoch)+".mat" %保存路径

    for i = 1:batch
        raseed = raseed_arr(i);
        index = 5;
        mimo_uplink(i,:,:,:,:) = PUSCH(type,NRB,f_up,Vc(index),UEAtNum,BSAtNum,raseed); %上行频段2.0GHz
        mimo_downlink(i,:,:,:,:) = PDSCH(type,NRB,f_down,Vc(index),BSAtNum,UEAtNum,raseed); %下行频段2.1GHz
        fprintf("epoch:%d/%d,第%d组数据已生成\n",j,epoch,i)
        if(mod(i,batch/2) ==0)
            %压缩保存数据
            save(filename,'mimo_uplink','mimo_downlink','-v7.3')
            %不压缩保存数据
            %save(filename,'mimo_uplink','mimo_downlink','-v7.3','-nocompression')
...
save(filename2,'mimo_uplink','mimo_downlink','-v7.3')
m = matfile(filename2,'Writable',true)

i= 1;
 filename1 = "../data/v120"+type+"_"+num2str(carrnum)+"_"+num2str(BSAtNum)+"_"+num2str(UEAtNum)+"_"...
