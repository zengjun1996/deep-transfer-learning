%hPDSCHResources 5G NR PDSCH and DM-RS resource element indices and DM-RS values
%   [IND,DMRSIND,DMRS,INFO] = hPDSCHResources(GNB,CHS) returns the 
%   resource element (RE) indices for a 5G NR PDSCH transmission and 
%   associated PDSCH DM-RS, given the time (symbols) and frequency (PRBs)
%   allocation of the PDSCH and the DM-RS configuration. The 1-based linear
%   PDSCH indices are returned in matrix, IND. They are defined relative
%   to a three-dimensional RE grid representing a 14/12-symbol slot for the
%   full carrier (in the PDSCH numerology) across the layers/DM-RS ports
%   of the PDSCH. Each column of IND represents the grid locations for a
%   separate layer/port (the third dimension of the grid). The DM-RS RE
%   indices have the same format and are returned in matrix, DMRSIND. The
%   complex values of DM-RS sequence are also returned in matrix, DMRS. 
%  
...
%   signaled configuration types ('dmrs-Type'). Configuration
...
%   the allocated PDSCH symbols, or 1 or 2 pairs of neighbouring or 'double 
...
%   parameters is given by TS 38.211 tables 7.4.1.1.2-3 and 7.4.1.1.2-4.
%   In this function, the value of the 'DMRSLength' input parameter
...
%   slotgrid = zeros(12*gnb.NRB,symbperslot,length(pdsch.PortSet));
%   [ind,dmrsind,dmrs,info] = hPDSCHResources(gnb,pdsch);
...
function [pdschIndices,dmrsIndices,dmrssymb,pdschInfo] = hPDSCHResources(gnb,pdsch)
    ...
    [pdschIndices,dmrsIndices,dmrssymb,pdschInfo] = hSharedChannelResources(ftable,gnb,pdsch);
